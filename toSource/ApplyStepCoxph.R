ApplyStepCoxph <- function(trainingTestStructure,dataOptionsStructure,parameterStructure,plotSaveOptions){
	#-------------------------------------------------------------------------------------------------------#
	# K Lloyd 2017_08_09
	#-------------------------------------------------------------------------------------------------------#
	...
	# Forward selection feature selection implemented via stepwise selection, using stepAIC
	#-------------------------------------------------------------------------------------------------------#
	...
	#-------------------------------------------------------------------------------------------------------#
	model.coxph 		<- coxph(Surv(y,event)~1,data=trainingAll)
	model.step.coxph 	<- stepAIC(model.coxph,direction='forward',k=log(dim(trainingTestStructure$trainingData)[1]),
							scope=list(lower=model.coxph,upper=paste0('~',paste0('x',1:dimension,collapse='+'))))
	modelCoefficients 	<- model.step.coxph$coefficients
	trainingPredictions <- predict(model.step.coxph,type='lp')
	testPredictions 	<- predict(model.step.coxph, subset(testAll,select=names(modelCoefficients)),type='lp')
	...
	modelVariables 		<- names(coef(model.step.coxph))[which(coef(model.step.coxph)!=0)]
