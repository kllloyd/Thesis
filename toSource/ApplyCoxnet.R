ApplyCoxnet <- function(trainingTestStructure,dataOptionsStructure,parameterStructure,plotSaveOptions){
    #-------------------------------------------------------------------------------------------------------#
    # K Lloyd 2016_09_16
    #-------------------------------------------------------------------------------------------------------#
	# Predicting risk of data set members given censored and uncensored training data.
	# Applied Cox Proportional Hazards with Elastic-net Penalisation using coxph and glmnet
	#-------------------------------------------------------------------------------------------------------#
	...
	if(printOptions){
		...
		cat('\tPrediction was using Coxnet',fill=TRUE)
		cat('-------------------------------------------------------',fill=TRUE)
	}
	...
	modelCoefficients.glmnet 		<- as.matrix(coef(model.glmnet, s = model.glmnet$lambda[which.max(model.glmnet$dev.ratio)]))
	...
	if(saveFiles){
		sink(paste0(fileName,'CoxnetMetrics.txt'),append=TRUE)
			cat('Model = Coxnet',fill=TRUE)
			cat('Time taken = ',timeTaken,fill=TRUE)
			cat('C Index = ',c.index,fill=TRUE)
			cat('-------------------------------------------------------',fill=TRUE)
		sink()
		write.table(testPredictions,paste0(fileName,'CoxnetTestTargetPredictions.csv'),sep=',',quote=FALSE,row.names=FALSE)
		write.table(testAll$y,paste0(fileName,'CoxnetTestTargets.csv'),sep=',',quote=FALSE,row.names=FALSE)
	}
	...
	toReturn = list('timeTaken'=timeTaken,'trainingPredictions'=trainingPredictions,'testPredictions'=testPredictions,'c.index'=c.index,'rmse'=rmse,'brier'=brier,
					'model'='Coxnet','modelVariables'=modelVariablesSelected,'trainingTestStructure'=trainingTestStructure)
